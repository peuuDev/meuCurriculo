...
public class CarrinhoDeCompra2 {
    ...
    static String[] nomes = {"Arroz 5kg", "Feijão Carioca 1kg", "Macarrão Espaguete 500g", "Açúcar Refinado 1kg",
            "Café Torrado 500g", "Leite Integral 1L", "Óleo de Soja 900ml", "Farinha de Trigo 1kg",
            "Sal Refinado 1kg", "Molho de Tomate 340g", "Biscoito Recheado 130g", "Papel Higiênico (4 unid)",
            "Sabonete 90g", "Detergente Líquido 500ml", "Sabão em Pó 1kg", "Refrigerante 2L",
            "Água Mineral 1,5L", "Margarina 500g", "Queijo Mussarela 500g", "Presunto 500g"};
    ...
    public static void exibirCarrinho() {
        ...
        for (int i = 0; i < itensCarrinho; i++) {
            System.out.println(i + 1 + " - " + carrinhoNome[i] + " Quantidade: " + carrinhoQntd[i] + " | Preço: R$ " + carrinhoPreco[i]);
    ...
    public static void removerItem() {
        if (itensCarrinho == 0) {
            System.out.println("Carinho está vazio!");
        }
        exibirCarrinho();
        ...
        double qntdRemovida = carrinhoQntd[indice];
        String nomeRemovido = carrinhoNome[indice];

        for (int i = indice; i < itensCarrinho - 1; ) {

            carrinhoNome[i] = carrinhoNome[i + 1];
            carrinhoPreco[i] = carrinhoPreco[i + 1];
            carrinhoQntd[i] = carrinhoQntd[i + 1];

        }
        carrinhoNome[itensCarrinho - 1] = null;
        carrinhoPreco[itensCarrinho - 1] = 0.0;
        carrinhoQntd[itensCarrinho - 1] = 0;
        itensCarrinho++;
